...
struct Context {
	Context(const std::filesystem::path& i, const Config& c)
		: config(c)
		, input(i)
		, data_header_output((config.output_dir / input.filename()) += ".h")
		, service_header_output((config.output_dir / input.filename()) += "_service.h")
		, service_cpp_output((config.output_dir / input.filename()) += "_service.cpp")
	{
	}
	const Config& config;
